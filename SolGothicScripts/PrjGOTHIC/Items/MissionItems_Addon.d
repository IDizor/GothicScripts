...
func void Use_Addon_Stone_01()
{
	...
	if(PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines(nDocID,0,"Мы, последние Трое из Совета Пяти, установили в храме первую Ловушку, скрывающую вход, чтобы меч больше никогда не увидил Дневной Свет.");
	}
	else
	{
...
func void Use_Addon_Stone_05()
{
	...
	if(PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines(nDocID,0,"Я, тот, кто был против решения Совета Трех, создал вторую ловушку. Лишь я знаю в какую из дверей нужно идти.");
	}
	else
	{
...
func void Use_Addon_Stone_03()
{
	...
	if(PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines(nDocID,0,"КАРДИМОН создал третью ловушку. Лишь тот, кто следует по пути света, достигнет третьего зала.");
	}
	else
	{
...
func void Use_Addon_Stone_04()
{
	...
	if(PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines(nDocID,0,"Последнюю ловушку создал КУАРХОДРОН. Лишь он знает как не умереть, открывая проход.");
	}
	else
	{
...
func void Use_Addon_Stone_02()
{
	...
	if(PLAYER_TALENT_FOREIGNLANGUAGE[LANGUAGE_1] == TRUE)
	{
		Doc_PrintLines(nDocID,0,"Внешние ворота запечатали КАРДИМОН и КУАРХОДРОН. Этот ритуал стал для них смертельным.");
		Doc_PrintLine(nDocID,0,"");
