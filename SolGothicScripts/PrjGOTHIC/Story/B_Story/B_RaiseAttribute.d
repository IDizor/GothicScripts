...
func int GetManaMiltiplier()
{
	if((hero.guild == GIL_KDF) || hero.guild == GIL_NOV)
	{
		return ManaUpFor1LP_Mage;
	}
	else
	{
		return ManaUpFor1LP_Regular;
	};
};

func int ManaPoints(var int rawValue)
{
	var int resultPoints;
	resultPoints = rawValue * GetManaMiltiplier();
	return resultPoints;
};

func int ManaLearnCost(var int manaPoints)
{
	var int resultCost;
	resultCost = manaPoints / GetManaMiltiplier();
	return resultCost;
};
