...
{
	AI_Output(other,self,"DIA_Addon_Edgor_Found_15_00");	//Я нашел каменную табличку!
	AI_Output(self,other,"DIA_Addon_Edgor_Found_06_01");	//Правда? Ты смелый парень.
	AI_Output(self,other,"DIA_Addon_Edgor_Found_06_02");	//Тогда ты наверняка заработал себе пропуск в лагерь.
};
...
func void DIA_Addon_Edgor_Teach_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Edgor_Teach_06_02");	//Но я знаю, как отрывать от них жала и крылья, от их мертвых тушек. Да! Отрывать от них...
...
func int GetBloodflyLearnCost()
{
	if((Fortuno_Done == FALSE) && Npc_KnowsInfo(other,DIA_Addon_Fortuno_FREE))
	{
		return 0;
	}
	else
...
func string GetBloodflyCostString()
{
	var string tempStr;
	var int tempCost;
	tempCost = GetBloodflyLearnCost();
	if(tempCost == 0)
	{
		tempStr = "";
	}
	...
	{
		tempStr = ConcatStrings(IntToString(tempCost)," LP, ");
	};
	return tempStr;
};
...
func void DIA_Addon_Edgor_Start_Info()
{
	...
	{
		Info_AddChoice(DIA_Addon_Edgor_TrainStart,ConcatStrings(ConcatStrings("Секрет из жала (",GetBloodflyCostString()),"100 золота)"),DIA_Addon_Edgor_TrainStart_GIFT);
	};
};
...
func void DIA_Addon_Edgor_TrainStart_GIFT()
{
	if(B_GiveInvItems(other,self,ItMi_Gold,100))
	{
		var int bloodflyLpCost;
		bloodflyLpCost = GetBloodflyLearnCost();
		if(other.lp >= bloodflyLpCost)
		{
			...
			other.lp = other.lp - bloodflyLpCost;
			Knows_Bloodfly = TRUE;
