...
func void DIA_Kardif_Hallo_Zehn()
{
	...
	AI_Output(self,other,"DIA_Kardif_Hallo_Zehn_14_01");	//Ты заключил выгодную сделку. Я всегда к твоим услугам.
	Kardif_Deal = 10;
	Info_ClearChoices(DIA_Kardif_Hallo);
};
...
func void DIA_Kardif_Hallo_Angebot()
{
	...
	AI_Output(self,other,"DIA_Kardif_Hallo_Angebot_14_01");	//Что, 5 золотых монет? Ты хочешь разорить меня? - Давай сойдемся на 7.
	Info_ClearChoices(DIA_Kardif_Hallo);
...
func void DIA_Kardif_Hallo_Sieben()
{
	...
	AI_Output(self,other,"DIA_Kardif_Hallo_Sieben_14_01");	//Мудрое решение. Ну, если тебе нужно будет что-то узнать, обращайся ко мне.
	Kardif_Deal = 7;
	Info_ClearChoices(DIA_Kardif_Hallo);
};
...
func void DIA_Kardif_Hallo_Ablehnen()
{
	...
	AI_Output(self,other,"DIA_Kardif_Hallo_Ablehnen_14_01");	//Эй, подожди... - хорошо, 5 золотых монет. Но это моя последняя цена!
	Info_ClearChoices(DIA_Kardif_Hallo);
...
func void DIA_Kardif_Lehmar_Info()
{
	if(B_GiveInvItems(other,self,ItMi_Gold,Kardif_Deal))
	{
		...
		AI_Output(self,other,"DIA_Kardif_Add_14_01");	//Если ты хочешь узнать это, тебе стоит заглянуть в его гроссбух.
...
func void DIA_Kardif_Lernen_Info()
{
	if(B_GiveInvItems(other,self,ItMi_Gold,Kardif_Deal))
	{
		...
		AI_Output(self,other,"DIA_Kardif_Lernen_14_06");	//Боже, король успеет выиграть войну против орков, прежде чем я тебе объясню все это.
		...
		AI_Output(self,other,"DIA_Kardif_Lernen_14_08");	//Его совсем легко найти. Он живет в соседнем доме направо от моего трактира. Я должен был взять в два раза больше за эту информацию.
...
func void DIA_Kardif_SENDATTILA_Info()
{
	...
	AI_Output(self,other,"DIA_Kardif_SENDATTILA_14_06");	//Я мог бы сказать тебе - но тебе придется заплатить за это, мой друг.
	if(Kardif_Deal == 0)
	{
		AI_Output(self,other,"DIA_Kardif_Hallo_14_04");	//Каждая информация, которую я сообщу тебе, будет стоить 10 золотых монет.
		Kardif_Deal = 10;
