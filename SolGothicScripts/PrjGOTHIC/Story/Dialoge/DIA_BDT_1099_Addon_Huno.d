...
func void DIA_Addon_Huno_Attentat_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Huno_Attentat_06_00");	//Что тебе от меня нужно?
...
{
	AI_Output(self,other,"DIA_Addon_Huno_Stress_06_00");	//А теперь послушай меня. Двигал бы ты свою задницу отсюда, пока я ее не порвал.
	Huno_Angepisst = TRUE;
};
...
func void DIA_Addon_Huno_SomeThings_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Addon_Huno_SomeThings_06_02");	//Опять? Надеюсь, оно того стоит?
	};
	Huno_Counter = 0;
	if(Finn_TellAll == TRUE)
	{
		AI_Output(other,self,"DIA_Addon_Huno_SomeThings_15_03");	//Я слышал, что тебя не было на месте во время нападения!
		AI_Output(self,other,"DIA_Addon_Huno_SomeThings_06_04");	//Что еще?
	...
	if(Paul_TellAll == TRUE)
	{
		...
		AI_Output(self,other,"DIA_Addon_Huno_SomeThings_06_06");	//Продолжай...
		Huno_Counter = Huno_Counter + 1;
	};
	if(Emilio_TellAll == TRUE)
	{
		...
		AI_Output(self,other,"DIA_Addon_Huno_SomeThings_06_08");	//Значит, ты болтал со стариком Эмилио?
		if(Huno_Counter > 0)
		{
		...
		{
			AI_Output(self,other,"DIA_Addon_Huno_SomeThings_06_11");	//Может, мне тоже стоит с ним поболтать? Уверен, что смогу переубедить старика.
			B_Addon_Huno_Stress();
			AI_StopProcessInfos(self);
		};
	}
...
func void DIA_Addon_Huno_SomeThings_Pro()
{
	...
	AI_Output(self,other,"DIA_Addon_Huno_SomeThings_Pro_06_01");	//Ты в самом деле такой идиот, или только кажешься? Думаешь, что сможешь подлизаться к Эстебану? 
...
func void DIA_Addon_Huno_SomeThings_TellMeNow()
{
	...
	AI_Output(self,other,"DIA_Addon_Huno_SomeThings_TellMeNow_06_04");	//Ладно. Я организую встречу. Но только так, как я этого хочу, понял?
