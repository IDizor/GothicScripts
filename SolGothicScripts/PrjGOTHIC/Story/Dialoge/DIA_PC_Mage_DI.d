...
func void DIA_Milten_DI_Rat_nein()
{
	...
	AI_Output(self,other,"DIA_Milten_DI_Rat_nein_03_01");	//Ты такой...Что ты будешь делать, если ты встретишься с драконами здесь, на острове?
...
func void DIA_Milten_DI_PEDROTOT_Info()
{
	...
	AI_Output(self,other,"DIA_Milten_DI_PEDROTOT_03_01");	//Что? Где? Здесь, на острове? Черт, но это абсолютно невероятно.
...
func void DIA_Milten_DI_TeachMagic_Info()
{
	...
	Info_AddChoice(DIA_Milten_DI_TeachMagic,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Milten_DI_TeachMagic_MANA_1);
	Info_AddChoice(DIA_Milten_DI_TeachMagic,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Milten_DI_TeachMagic_MANA_5);
...
{
	if(B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(1),T_HIGH * ManaTeachersLimitMultiplier))
	{
...
{
	if(B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(5),T_HIGH * ManaTeachersLimitMultiplier))
	{
...
func void DIA_Milten_DI_UndeadDragonDead_Info()
{
	...
	if(DIA_Milten_DI_UndeadDragonDead_OneTime == FALSE)
	{
		...
		AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_03");	//Мы, когда-нибудь сможем пожить спокойно? Мы, определенно, заслужили это.
		if(hero.guild == GIL_KDF)
		{
			...
			AI_Output(self,other,"DIA_Milten_DI_UndeadDragonDead_03_08");	//Сухо, как всегда. Эй, парень. Ты только что спас мир. Разве это недостаточная причина для радости?
