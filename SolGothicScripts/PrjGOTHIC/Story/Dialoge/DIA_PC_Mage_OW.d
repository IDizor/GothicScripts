...
func void DIA_MiltenOW_Hello_Info()
{
	...
	Info_AddChoice(DIA_MiltenOW_Hello,"Я что, знаю тебя?",DIA_MiltenOW_Hello_NO);
};
...
{
	if((KnowsAboutGorn == TRUE) && (Kapitel == 2) && (Npc_KnowsInfo(other,DIA_MiltenOW_Wo) || Knows_Diego == FALSE))
	{
		return TRUE;
	};
};
...
instance DIA_MiltenOW_TeachCircle2(C_Info)
{
	...
	permanent = TRUE;
	description = "Обучи меня второму кругу магии! (5 LP)";
};
...
func void DIA_MiltenOW_Mana_Info()
{
	...
	Info_AddChoice(DIA_MiltenOW_Mana,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_MiltenOW_Mana_1);
	Info_AddChoice(DIA_MiltenOW_Mana,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_MiltenOW_Mana_5);
};
