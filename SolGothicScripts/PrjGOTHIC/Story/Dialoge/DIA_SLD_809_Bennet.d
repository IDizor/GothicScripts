...
instance DIA_Bennet_Purchase_Steel(C_Info)
{
	npc = SLD_809_Bennet;
	nr = 701;
	condition = DIA_Bennet_Purchase_Steel_Condition;
	information = DIA_Bennet_Purchase_Steel_Info;
	permanent = TRUE;
	description = "(купить раскаленную сталь)";
};


func string DIA_Bennet_Purchase_Steel_Desc()
{
	var string desc;
	desc = ConcatStrings("(купить раскаленную сталь в наличии ",IntToString(HotSteelCount));
	desc = ConcatStrings(desc," шт.)");
	return desc;
};


func int DIA_Bennet_Purchase_Steel_Condition()
{
	if((Kapitel != 3) || (MIS_RescueBennet == LOG_SUCCESS))
	{
		HotSteelCount = Npc_HasItems(self,ItMiSwordrawhot);
		if(HotSteelCount > 0)
		{
			DIA_Bennet_Purchase_Steel.description = DIA_Bennet_Purchase_Steel_Desc();
			return TRUE;
		}
		else
		{
...
func void DIA_Bennet_Purchase_Steel_Info()
{
	var int steelCost;
	steelCost = HotSteelCount * 10;
	if(Npc_HasItems(other,ItMi_Gold) >= steelCost)
	{
		B_GiveInvItems(other,self,ItMi_Gold,steelCost);
		AI_Output(self,other,"DIA_Bennet_GetInnosEye_06_01");	//Да, держи.
		B_GiveInvItems(self,other,ItMiSwordrawhot,HotSteelCount);
		AI_StopProcessInfos(self);
	}
	else
	{
...
func void DIA_Bennet_WhichWeapons_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_WhichWeapons_06_03");	//Ты случайно не знаешь, где можно раздобыть руды? Кроме как в Долине Рудников, я имею в виду.
...
func void DIA_Bennet_WannaSmithORE_Info()
{
	...
	else if((other.guild != GIL_SLD) && (other.guild != GIL_DJG) && (MIS_RescueBennet != LOG_SUCCESS))
	{
		...
		{
			AI_Output(self,other,"DIA_Bennet_WannaSmithORE_06_06");	//Ничего личного. Против тебя я ничего не имею.
		};
	}
	...
	{
		if((other.guild != GIL_SLD) && (other.guild != GIL_DJG) && (MIS_RescueBennet == LOG_SUCCESS))
		{
			AI_Output(self,other,"DIA_Bennet_WannaSmith_06_01");	//Конечно.
...
func void DIA_Bennet_BringOre_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_BringOre_06_01");	//Покажи!
...
func void DIA_Bennet_TeachSmith_Info()
{
	...
	else if((Kapitel >= 5) && (Bennet_Kap5Smith == FALSE))
	{
		...
		AI_Output(self,other,"DIA_Bennet_TeachSmith_06_05");	//А ты хочешь узнать?
...
func void DIA_Bennet_ThankYou_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_ThankYou_06_03");	//Он был так напуган, что чуть не наложил в штаны.
	...
	AI_Output(self,other,"DIA_Bennet_ThankYou_06_06");	//Презент.
};
...
instance DIA_Bennet_DJG_ARMOR_M(C_Info)
{
	...
	permanent = TRUE;
	description = "Средние доспехи охотника на драконов: 120/120/75/35. Цена: 12000 золота.";
};
...
instance DIA_Bennet_DJG_ARMOR_H(C_Info)
{
	...
	permanent = TRUE;
	description = "Тяжелые доспехи охотника на драконов: 150/150/100/50. Цена: 20000 золота.";
};
...
{
	if(Npc_KnowsInfo(other,DIA_Bennet_RepairNecklace) && Npc_HasItems(other,ItMi_InnosEye_Broken_Mis) && (MIS_Bennet_InnosEyeRepairedSetting != LOG_SUCCESS))
	{
		return TRUE;
	};
};
...
{
	if(Npc_KnowsInfo(other,DIA_Bennet_ShowInnosEye) && (Npc_HasItems(other,ItMi_InnosEye_Broken_Mis) >= 1) && (MIS_SCKnowsInnosEyeIsBroken == TRUE) && (MIS_RescueBennet == LOG_SUCCESS) && (MIS_Bennet_InnosEyeRepairedSetting != LOG_SUCCESS))
	{
		return TRUE;
	};
};
...
func void DIA_Bennet_DRACHENEIER_mehr()
{
	...
	AI_Output(self,other,"DIA_Bennet_DRACHENEIER_mehr_06_01");	//Хорошо. 350 и точка. Я не могу дать тебе больше - иначе это дело не окупится.
...
func void DIA_Bennet_StillNeedYou_Info()
{
	...
	AI_Output(self,other,"DIA_Bennet_StillNeedYou_06_01");	//Хорошо! Всякий может издеваться над простым кузнецом! Увидимся в гавани.
	self.flags = NPC_FLAG_IMMORTAL;
	Bennet_IsOnBoard = LOG_SUCCESS;
	Crewmember_Count = Crewmember_Count + 1;
	AI_StopProcessInfos(self);
