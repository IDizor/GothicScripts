...
func void DIA_Alrik_Hallo_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Alrik_Hallo_09_01");	//Это мой дом!
	}
...
func void DIA_Alrik_Regeln_Info()
{
	...
	AI_Output(self,other,"DIA_Alrik_Regeln_09_04");	//Ставка - 50 золотых. Если ты победишь, ты получишь 100 монет. Если нет - ну, тогда твои деньги останутся у меня.
...
func void DIA_Alrik_WannaFight_Gold()
{
	...
	AI_Output(self,other,"DIA_Alrik_WannaFight_Gold_09_01");	//У нас новый боец!
...
func void DIA_Alrik_AfterFight_Info()
{
	if((self.aivar[AIV_LastPlayerAR] == AR_NONE) && (self.aivar[AIV_ArenaFight] != AF_NONE) && (self.aivar[AIV_ArenaFight] != AF_AFTER_PLUS_DAMAGE))
	{
		...
		{
			AI_Output(self,other,"DIA_Alrik_AfterFight_09_06");	//Ты хорошо дрался. Но твоя ставка пропала - не расстраивайся особенно, она пойдет на доброе дело!
		}
		else
		{
...
{
	AI_Output(other,self,"DIA_Alrik_DuWohnst_15_00");	//Ты 'живешь' за этим складом?
	AI_Output(self,other,"DIA_Alrik_DuWohnst_09_01");	//Только временно. Когда имеешь столько денег, как у меня, можно позволить себе немного роскоши!
...
func void DIA_Alrik_WerSchwert_Info()
{
	...
	AI_Output(self,other,"DIA_Alrik_WerSchwert_09_04");	//Это старый меч. Он вряд ли много за него запросит. Просто думай об этих деньгах, как о дополнительной ставке.
...
func void DIA_Alrik_Ausbilden_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Alrik_Ausbilden_09_01");	//Если ты действительно хочешь научиться сражаться, то выходи против меня. За этот урок я не возьму дополнительной платы.
