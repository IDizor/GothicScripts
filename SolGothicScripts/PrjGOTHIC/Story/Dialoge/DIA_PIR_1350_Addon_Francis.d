...
func void DIA_Addon_Francis_AboutGreg_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Francis_AboutGreg_13_03");	//А он оставил за главного МЕНЯ!
...
func void DIA_Addon_Francis_Key_2000Gold()
{
	...
	{
		AI_Output(self,other,"DIA_Addon_FrancisNotEnoughGold_13_05");	//Ха! Готов поспорить, у тебя столько нет!
	};
	Info_ClearChoices(DIA_Addon_Francis_Key);
};
...
func void DIA_Addon_Francis_Buch_Info()
{
	...
	if(Npc_HasItems(self,ITKE_Greg_ADDON_MIS))
	{
		...
		{
			AI_Output(self,other,"DIA_Addon_Francis_Buch_13_05");	//Тебе же нужен был ключ от хижины Грега, не так ли?
		};
		...
		AI_Output(self,other,"DIA_Addon_Francis_Buch_13_08");	//Но ты должен отдать мне книгу и держать свой рот на замке, идет?
	...
	else
	{
		...
		AI_Output(self,other,"DIA_Addon_Francis_Buch_13_13");	//А теперь давай мне сюда эту штуку...
		B_GiveInvItems(other,self,ITWR_Addon_FrancisAbrechnung_Mis,1);
	};
	AI_StopProcessInfos(self);
};
...
func void DIA_Francis_Ausgeschissen_Info()
{
	if(!Npc_IsDead(Greg))
	{
		AI_Output(self,other,"DIA_Addon_Francis_Ausgeschissen_13_00");	//Благодаря тебе Грег заставил меня пилить дрова до тех пор, пока на острове не останется ни одного дерева.
	};
