...
func void DIA_Mil_333_Stadtwache_FirstWarn_Info()
{
	...
	{
		AI_Output(other,self,"DIA_Mil_333_Stadtwache_FirstWarn_15_01");	//Что еще?
	...
	else
	{
		...
		if((Npc_HasEquippedArmor(other) == FALSE) || (Hlp_IsItem(itm,ITAR_Bau_L) == TRUE) || (Hlp_IsItem(itm,ITAR_Bau_M) == TRUE))
		{
			...
			if(Npc_KnowsInfo(other,DIA_Mil_310_Stadtwache_MilizWerden))
			{
				...
				AI_Output(self,other,"DIA_Mil_333_Stadtwache_FirstWarn_06_14");	//Ох, парень! Они всегда посылают к нам сливки общества...
			};
		}
		else
		{
			AI_Output(other,self,"DIA_Mil_333_Stadtwache_FirstWarn_15_15");	//Что?
...
func void DIA_Mil_333_Stadtwache_Bribe_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Mil_333_Stadtwache_Bribe_06_01");	//100 золотых - это неплохо. Проходи, конечно.
		if(B_GetGreatestPetzCrime(self) >= CRIME_ATTACK)
		{
