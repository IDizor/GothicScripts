...
func void DIA_Addon_Matt_Anheuern_ShutUp()
{
	...
	AI_Output(self,other,"DIA_Addon_Matt_FollowMe_ShutUp_10_01");	//Есть, капитан!
	Info_ClearChoices(DIA_Addon_Matt_Anheuern);
	AI_StopProcessInfos(self);
...
func void DIA_Addon_Matt_Anheuern_ClearCanyon()
{
	...
	AI_Output(self,other,"DIA_Addon_Matt_FollowMe_ClearCanyon_10_03");	//Надеюсь, у тебя достаточно бутылок с лечебным зельем. Они нам понадобятся.
	Info_ClearChoices(DIA_Addon_Matt_Anheuern);
	AI_StopProcessInfos(self);
	Npc_ExchangeRoutine(self,"FOLLOW");
	self.aivar[AIV_PARTYMEMBER] = TRUE;
...
func void DIA_Addon_Matt_GoHome_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Matt_GoHome_10_01");	//Мне не помешает выпить кружечку грога!
	AI_StopProcessInfos(self);
...
{
	AI_Output(self,other,"DIA_Addon_Matt_TooFar_10_01");	//Дальше тебе придется идти одному, капитан.
