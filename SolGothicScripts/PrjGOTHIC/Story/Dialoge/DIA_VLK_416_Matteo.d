...
instance DIA_Matteo_LEATHER(C_Info)
{
	...
	permanent = TRUE;
	description = "Купить кожаные доспехи: 25/20/5/0. Цена: 250 золота.";
};


func int DIA_Matteo_LEATHER_Condition()
{
	if(Npc_KnowsInfo(other,DIA_Matteo_SellWhat) && (Matteo_LeatherBought == FALSE) && (other.guild == GIL_NONE))
	{
		return TRUE;
	};
};
...
func void DIA_MAtteo_HelpMeToOV_Info()
{
	...
	AI_Output(self,other,"DIA_Matteo_HelpMeToOV_09_01");	//Что? А что тебе нужно ТАМ?
	...
	{
		AI_Output(other,self,"DIA_Matteo_HelpMeToOV_15_04");	//Ох, забудь об этом!
		AI_Output(self,other,"DIA_Matteo_HelpMeToOV_09_05");	//Это так ТИПИЧНО для этих нищих!
	}
	else
	{
...
func void B_Matteo_Preis()
{
	...
	AI_Output(self,other,"DIA_Matteo_HelpMeNow_09_02");	//Вопрос в том: НАСКОЛЬКО это важно для тебя?
...
func void DIA_MAtteo_Minenanteil_Info()
{
	...
	AI_Output(self,other,"DIA_Matteo_Minenanteil_09_01");	//Акции? Ох. И откуда они взялись? Понятия не имею, где я взял их. Клянусь, ваша честь.
