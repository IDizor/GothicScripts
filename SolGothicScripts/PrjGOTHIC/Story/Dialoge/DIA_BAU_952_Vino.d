	...
	if(hero.guild == GIL_NONE)
	{
		...
		if(!Npc_IsDead(Lobart))
		{
			...
			if(((Mob_HasItems("CHEST_LOBART",ITAR_Bau_L) == TRUE) || (Npc_HasItems(Lobart,ITAR_Bau_L) > 0)) && (Lobart_Kleidung_Verkauft == FALSE) && (Npc_KnowsInfo(other,DIA_Lobart_KLEIDUNG) || Npc_KnowsInfo(other,DIA_Lobart_WorkNOW)))
			{
				...
				AI_Output(self,other,"DIA_Vino_SeekWork_05_07");	//Принеси мне бутылку вина, и я скажу Лобарту, что ты очень помог нам.
...
func void DIA_Vino_BringWine_Info()
{
	...
	AI_Output(self,other,"DIA_Vino_BringWine_05_01");	//Я, пожалуй, не буду спрашивать, где ты взял его, хорошо? Кого это волнует?
	if(!Npc_IsDead(Lobart))
	{
...
func void DIA_Vino_Obesessed_Info()
{
	...
	else
	{
		...
		AI_Output(self,other,"DIA_Vino_Obesessed_05_01");	//Черт, убей их. Иначе они убьют меня.
