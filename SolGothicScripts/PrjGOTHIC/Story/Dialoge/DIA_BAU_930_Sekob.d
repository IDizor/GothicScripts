...
func void DIA_Sekob_Defeated_Info()
{
	...
	AI_Output(self,other,"DIA_Sekob_Defeated_01_03");	//Но, господин, у меня ничего нет. Я простой бедняк, я сам пухну от голода.
...
func void B_Sekob_Kassieren()
{
	...
	AI_Output(self,other,"DIA_Sekob_Kassieren_01_01");	//Нет, пожалуйста, вот, возьми золото. Я даже прибавлю сверху, если ты оставишь меня в живых.
	B_GiveInvItems(self,other,ItMi_Gold,60);
	AI_Output(other,self,"DIA_Sekob_Kassieren_15_02");	//Вот видишь, это было не так уж и сложно.
	AI_Output(self,other,"DIA_Sekob_Kassieren_01_03");	//Я разорен.
...
func void DIA_Sekob_PERMKAP1_Info()
{
	...
	AI_Output(self,other,"DIA_Sekob_PERMKAP1_01_01");	//Что я сделал, чтобы заслужить такое...
	AI_StopProcessInfos(self);
};
...
func void DIA_Sekob_Heilung_Info()
{
	...
	if(DIA_Sekob_Heilung_oneTime == FALSE)
	{
		...
		AI_Output(other,self,"DIA_Sekob_Heilung_15_02");	//Тяжелый случай.
		B_NpcClearObsessionByDMT(self);
	...
	else
	{
		...
		AI_Output(other,self,"DIA_Sekob_Heilung_15_04");	//Безнадежный случай.
		B_NpcClearObsessionByDMT(self);
		B_Attack(self,other,AR_NONE,1);
	};
};
