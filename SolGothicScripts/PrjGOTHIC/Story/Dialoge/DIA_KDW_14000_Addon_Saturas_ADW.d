...
func void DIA_Addon_Saturas_ADWStart_RavenAim()
{
	...
	AI_Output(self,other,"DIA_Addon_Bodo_14_02");	//Естественно, мы не ждем от него ничего хорошего...
};
...
func void DIA_Addon_Saturas_ADWStart_wastun2()
{
	...
	AI_Output(other,self,"DIA_Addon_Saturas_ADWStart_wastun2_15_08");	//И это все? Я сделаю все это одной левой.
	AI_Output(self,other,"DIA_Addon_Saturas_ADWStart_wastun2_14_09");	//Я знаю, что прошу слишком о многом. Воспринимай это как шанс вернуть мое доверие.
...
func void DIA_Addon_Saturas_Tokens_Info()
{
	...
	if((Npc_HasItems(other,ItMi_Addon_Stone_01) && (Saturas_SCFound_ItMi_Addon_Stone_01 == FALSE)) || (Npc_HasItems(other,ItMi_Addon_Stone_02) && (Saturas_SCFound_ItMi_Addon_Stone_02 == FALSE)) || (Npc_HasItems(other,ItMi_Addon_Stone_03) && (Saturas_SCFound_ItMi_Addon_Stone_03 == FALSE)) || (Npc_HasItems(other,ItMi_Addon_Stone_04) && (Saturas_SCFound_ItMi_Addon_Stone_04 == FALSE)) || (Npc_HasItems(other,ItMi_Addon_Stone_05) && (Saturas_SCFound_ItMi_Addon_Stone_05 == FALSE)))
	{
		...
		if(Npc_HasItems(other,ItMi_Addon_Stone_01) && (Saturas_SCFound_ItMi_Addon_Stone_01 == FALSE))
		{
			...
			AI_Output(self,other,"DIA_Addon_Saturas_Tokens_14_09");	//Бьюсь об заклад, бандиты даже не знают, что они используют вместо денег.
...
func void DIA_Addon_Saturas_RavenInfos_Info()
{
	...
	if((SC_KnowsRavensGoldmine == TRUE) && (DIA_Addon_Saturas_RavenInfos_OneTime2 == FALSE))
	{
		...
		AI_Output(other,self,"DIA_Addon_Saturas_RavenInfos_15_07");	//Да, конечно. Я работаю над этим.
	...
	if((RavenIsInTempel == TRUE) && (DIA_Addon_Saturas_RavenInfos_OneTime4 == FALSE))
	{
		...
		AI_Output(self,other,"DIA_Addon_Saturas_RavenInfos_14_18");	//Что?! Это катастрофа! Почему ты не последовал за ним?!
...
func void DIA_Addon_Saturas_TuerZu_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Saturas_TuerZu_14_11");	//Ритуал...
...
func void DIA_Addon_Saturas_ADW_PreTeachCircle_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Saturas_ADW_PreTeachCircle_14_03");	//Вижу, ты серьезно настроен учиться. Что ж, я выполню твою просьбу.
...
instance DIA_Addon_Saturas_ADW_CIRCLE(C_Info)
{
	...
	permanent = TRUE;
	description = "Я хочу перейти на следующий уровень магии. (5 LP)";
};
...
func void DIA_Addon_Saturas_ADW_CIRCLE_Info()
{
	...
	else if((Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 2) && (Kapitel >= 2))
	{
	...
	else if((Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 3) && (Kapitel == 3))//(MIS_ReadyforChapter4 == TRUE))
	{
	...
	else if((Npc_GetTalentSkill(hero,NPC_TALENT_MAGE) == 4) && (MIS_ReadyforChapter4 == TRUE))//(Kapitel >= 4))
	{
