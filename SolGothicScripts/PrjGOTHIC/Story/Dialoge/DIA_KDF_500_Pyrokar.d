...
func void DIA_Pyrokar_WELCOME_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_WELCOME_11_01");	//Ты же знаешь, что все в Братстве Огня должны выполнять свои обязанности согласно воле Инноса.
};
...
{
	AI_Output(self,other,"DIA_Pyrokar_GOAWAY_11_00");	//Смирение - благодетель, которой тебе еще предстоит научиться. Так или иначе.
	AI_StopProcessInfos(self);
};
...
func void DIA_Pyrokar_FIRE_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_FIRE_11_01");	//Так ты знаешь о... ты хочешь пройти Испытание Огнем?
	AI_Output(other,self,"DIA_Pyrokar_FIRE_15_02");	//Да, я ссылаюсь на Закон Огня, который гласит...
	AI_Output(self,other,"DIA_Pyrokar_FIRE_11_03");	//Мы знаем Закон Огня. Мы также видели, как множество послушников погибли при прохождении этого испытания. Ты должен пересмотреть свое решение.
	AI_Output(other,self,"DIA_Pyrokar_FIRE_15_04");	//Я хочу пройти это испытание - и я пройду его.
	AI_Output(self,other,"DIA_Pyrokar_FIRE_11_05");	//Если ты категорически настаиваешь, Высший Совет подвергнет тебя этому испытанию.
...
func void DIA_Pyrokar_TEST_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_TEST_11_02");	//Испытанию Магией. Ты, вероятно, знаешь, что только ОДИН из послушников сможет пройти это испытание.
...
func void DIA_Pyrokar_SUCCESS_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_SUCCESS_11_01");	//Ты... сделал это?! Ты следовал знакам и нашел скрытый портал...
...
func void DIA_Pyrokar_MAGICAN_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_MAGICAN_11_02");	//Мы также уверены, что ты продолжишь прилагать все усилия, чтобы стать ДОСТОЙНЫМ слугой Инноса.
...
func void DIA_Pyrokar_OATH_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_OATH_11_02");	//Клянешься ли ты, перед лицом Инноса Всемогущего, его слуг и Священного Огня...
	...
	Fire_Contest = TRUE;
	B_RaiseAttribute(other,ATR_MANA_MAX,KlosterGuildManaBonus);
	Npc_ChangeAttribute(other,ATR_MANA,KlosterGuildManaBonus);
	Snd_Play("LEVELUP");
...
func void DIA_Pyrokar_Wunsch_Nothing()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_Wunsch_Nothing_11_01");	//Да будет так. Новый маг отказывается от первого шага.
...
instance DIA_Pyrokar_TEACH(C_Info)
{
	...
	permanent = TRUE;
	description = "Обучи меня последнему Кругу Магии. (5 LP)";
};
...
func void DIA_Pyrokar_TEACH_MANA_Info()
{
	...
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Pyrokar_TEACH_MANA_1);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Pyrokar_TEACH_MANA_5);
};

func void DIA_Pyrokar_TEACH_MANA_BACK()
{
	if(other.attribute[ATR_MANA_MAX] >= T_MEGA * ManaTeachersLimitMultiplier)
	{
...
{
	B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(1),T_MEGA * ManaTeachersLimitMultiplier);
	Info_ClearChoices(DIA_Pyrokar_TEACH_MANA);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,Dialog_Back,DIA_Pyrokar_TEACH_MANA_BACK);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Pyrokar_TEACH_MANA_1);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Pyrokar_TEACH_MANA_5);
...
{
	B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(5),T_MEGA * ManaTeachersLimitMultiplier);
	Info_ClearChoices(DIA_Pyrokar_TEACH_MANA);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,Dialog_Back,DIA_Pyrokar_TEACH_MANA_BACK);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Pyrokar_TEACH_MANA_1);
	Info_AddChoice(DIA_Pyrokar_TEACH_MANA,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Pyrokar_TEACH_MANA_5);
...
func void DIA_Pyrokar_BUCHZURUECK_Info()
{
	...
	AI_Output(self,other,"DIA_Pyrokar_BUCHZURUECK_11_03");	//Но это невероятно! Ты имеешь хоть малейшее представление о том, что ты только что дал мне?
	AI_Output(other,self,"DIA_Pyrokar_BUCHZURUECK_15_04");	//Ээээ. Нет.
	AI_Output(self,other,"DIA_Pyrokar_BUCHZURUECK_11_05");	//Это древний, давно потерянный том из давно прошедших дней.
...
{
	if((Kapitel == 5) && (Npc_KnowsInfo(other,DIA_Pyrokar_DRACHENTOT)))
	{
		return TRUE;
	};
};
...
func void DIA_Pyrokar_WASISTIRDORATH_Info()
{
	...
	AI_Output(other,self,"DIA_Pyrokar_WASISTIRDORATH_15_07");	//Растворился. Целый храм. Да, точно.
