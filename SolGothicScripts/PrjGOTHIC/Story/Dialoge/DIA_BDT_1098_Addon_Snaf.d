...
func void DIA_Addon_Snaf_Abrechnung_Info()
{
	...
	else
	{
		if(Finn_Petzt == TRUE)
		{
			...
			AI_Output(self,other,"DIA_Addon_Snaf_Abrechnung_01_10");	//Хотя вполне возможно, что он бросится к Эстебану с этой информацией. Ну, хотя ты сам все прекрасно понимаешь. 
		};
	...
	if(Snaf_Rechnung == FALSE)
	{
		if(Snaf_Einmal == FALSE)
		{
			...
			AI_Output(self,other,"DIA_Addon_Snaf_Abschied_01_15");	//Эй! Ты от меня ничего не слышал, ясно?!
			Snaf_Einmal = TRUE;
		}
		else
		{
...
func void DIA_Addon_Snaf_People_Huno()
{
	...
	AI_Output(self,other,"DIA_Addon_Snaf_People_Huno_01_02");	//Конечно, денег я за это с тебя не возьму.
};
