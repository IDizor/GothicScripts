...
{
	AI_Output(self,other,"DIA_Addon_Xardas_Hello_14_00");	//Наконец-то! Я и не думал, что нам с тобой доведется встретиться снова.
...
func void DIA_Addon_Xardas_Hello_Dragons()
{
	...
	AI_Output(other,self,"DIA_Xardas_Hello_15_10");	//Драконы!
	...
	{
		AI_Output(self,other,"DIA_Xardas_Hello_14_15");	//У нас очень мало времени.
	}
	else
	{
...
{
	if((RavenIsDead == TRUE) && (C_ScHasBeliarsWeapon() || (Saturas_KlaueInsMeer == TRUE)))
	{
		return TRUE;
	};
};
...
func void DIA_Xardas_KdfSecret_Info()
{
	...
	AI_Output(self,other,"DIA_Xardas_KdfSecret_14_04");	//Это преступление, которому в глазах Магов Огня, 'Посвященных Инноса', всегда таких 'Добрых' и 'Добродетельных', никогда не будет оправдания.
...
func void DIA_Xardas_RITUALREQUEST_Info()
{
	...
	AI_Output(self,other,"DIA_Xardas_RITUALREQUEST_14_03");	//Вот старый дьявол. Думаю, я знаю, что он затеял. Ты пришел, чтобы призвать меня к нему.
	...
	if((hero.guild == GIL_KDF) || (hero.guild == GIL_DJG) || (hero.guild == GIL_PAL))
	{
		...
		B_GivePlayerXP(XP_Ambient);
		if(DIA_Xardas_PYROWILLNICHT_Condition())
		{
...
func void DIA_Xardas_WARUMNICHTJETZT_Info()
{
	...
	AI_Output(self,other,"DIA_Xardas_WARUMNICHTJETZT_14_02");	//Не смей говорить со мной в таком тоне. Если бы не я, твое ни на что не годное тело до сих пор бы гнило в этом храме.
