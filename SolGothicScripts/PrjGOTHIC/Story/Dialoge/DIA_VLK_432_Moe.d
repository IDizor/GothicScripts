...
func void DIA_Moe_Hallo_Miliz()
{
	...
	AI_Output(self,other,"DIA_Moe_Hallo_Miliz_01_01");	//Ополчения здесь нет. А ты знаешь, почему их здесь не бывает?
	...
	AI_Output(self,other,"DIA_Moe_Hallo_Miliz_01_03");	//Они только иногда заглядывают в 'Красный Фонарь'. Понимаешь, здесь есть только ты и я.
};
...
func void DIA_Moe_Hallo_Ruhig()
{
	...
	AI_Output(self,other,"DIA_Moe_Hallo_Ruhig_01_01");	//Хорошо, но это будет стоить тебе 50 монет. Это плата за вход.
	Info_ClearChoices(DIA_Moe_Hallo);
	Info_AddChoice(DIA_Moe_Hallo,"Забудь об этом, ты не получишь ни цента!",DIA_Moe_Hallo_Vergisses);
	Info_AddChoice(DIA_Moe_Hallo,"Хорошо, я заплачу.",DIA_Moe_Hallo_Zahlen);
};
...
func void DIA_Moe_Hallo_Zahlen()
{
	...
	else
	{
		...
		AI_Output(self,other,"DIA_Moe_Hallo_Zahlen_01_06");	//Хорошо, ты можешь идти.
		AI_StopProcessInfos(self);
	};
};
...
func void DIA_Moe_Harbor_Rumors()
{
	...
	else if(Kapitel == 2)
	{
		...
		{
			AI_Output(self,other,"DIA_Moe_Harbor_Rumors_01_03");	//Все плохо. Времена настали тяжелые, но мы все еще пытаемся не сбиться с праведного пути.
