...
func void DIA_Wolf_Stadt_Info()
{
	...
	AI_Output(self,other,"DIA_Wolf_Stadt_08_01");	//В городе? Да меня туда силком не затащишь.
...
instance DIA_Wolf_TeachCrawlerPlates(C_Info)
{
	...
	permanent = TRUE;
	description = "Научи меня снимать панцири с краулеров!";
};
...
func int DIA_Wolf_TeachCrawlerPlates_Condition()
{
	...
	{
		DIA_Wolf_TeachCrawlerPlates.description = B_BuildLearnString("Научи меня снимать панцири с краулеров!",B_GetLearnCostTalent(other,NPC_TALENT_TAKEANIMALTROPHY,TROPHY_CrawlerPlate));
		return TRUE;
	};
};
...
func void DIA_Wolf_BENGAR_geld()
{
	...
	{
		//AI_Output(self,other,"DIA_Wolf_BENGAR_geld_08_02");	//Хорошо. Я иду туда. Посмотрим, что там за звери.
		AI_Output(self,other,"DIA_Wolf_BENGAR_geld_08_01");	//Увидимся позже.
	...
	{
		//AI_Output(self,other,"DIA_Wolf_BENGAR_geld_08_03");	//Ладно. Только не обманывай меня.
		AI_Output(self,other,"DIA_Wolf_BENGAR_geld_08_02");	//Если бы у тебя были деньги, я уже был бы там.
	};
	Info_ClearChoices(DIA_Wolf_BENGAR);
};
...
func void DIA_Wolf_KnowWhereEnemy_Info()
{
	...
	if(Crewmember_Count >= Max_Crew)
	{
		...
		AI_Output(self,other,"DIA_Wolf_KnowWhereEnemy_08_03");	//Так вот как ты поступаешь. Сначала зовешь меня с собой, а потом даешь от ворот поворот, да?
