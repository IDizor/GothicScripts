...
{
	AI_Output(self,other,"DIA_Addon_Senyan_Attack_12_00");	//Тогда зачем ты мне еще нужен, тунеядец?
	AI_Output(self,other,"DIA_Addon_Senyan_Attack_12_01");	//Эй, ребята, посмотрите, кто у нас здесь!
	Senyan_Called = TRUE;
...
func void B_Senyan_Erpressung()
{
	...
	AI_Output(self,other,"DIA_Addon_Senyan_Erpressung_12_01");	//О, нет, не надо, пожалуйста. Я не хочу вымогать у тебя деньги. Я даже и не мечтал об этом.
...
{
	if((MIS_Judas == 0) && Npc_KnowsInfo(other,DIA_Addon_BDT_1084_Senyan_Hi) && (Npc_IsDead(Esteban) == FALSE))
	{
		return TRUE;
	};
};
...
func void DIA_Addon_Senyan_Attentat_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Senyan_Attentat_12_06");	//И когда ты его найдешь - ты убьешь его для меня.
...
func void DIA_Addon_Senyan_Attentat_JA()
{
	...
	AI_Output(self,other,"DIA_Addon_Senyan_Attentat_JA_12_02");	//Когда ты выполнишь свою работу - и выполнишь ее хорошо - и выяснишь, кто виноват, мы убьем тупицу вместе. А теперь иди!
...
func int DIA_Addon_Senyan_derbe_Condition()
{
	...
	else if((Npc_KnowsInfo(other,DIA_Addon_BDT_1084_Senyan_Hi)) && (Npc_IsDead(Esteban)))
	{
		return TRUE;
	};
	return FALSE;
...
{
	AI_Output(self,other,"DIA_Addon_BDT_1084_Senyan_derbe_12_00");	//Ты не выполнил наше соглашение.
	B_Senyan_Attack();
};
