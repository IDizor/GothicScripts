...
func void DIA_Vatras_DI_HEAL_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Vatras_DI_HEAL_05_01");	//Аданос, благослови это тело. Ему предназначено восстановить баланс мира.
...
func void DIA_Vatras_DI_PEDROTOT_Info()
{
	...
	else
	{
		...
		if(Npc_IsDead(Pedro_DI))
		{
			...
			AI_Output(self,other,"DIA_Vatras_DI_PEDROTOT_05_07");	//Ох. Какая жалость. Я буду молиться за его бедную душу.
		}
		else
		{
			AI_StopProcessInfos(self);
...
{
	if(((VatrasPissedOffForever == TRUE) || ((MadKillerCount >= 15) && (VatrasMadKillerCount != MadKillerCount))) && Npc_IsInState(self,ZS_Talk))
	{
		return TRUE;
	};
};
