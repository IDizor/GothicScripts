...
{
	if((Kapitel >= 5) && (MadKillerCount >= 8) && (MadKillerCount < 15) && (VatrasPissedOffForever == FALSE) && Npc_IsInState(self,ZS_Talk))
	{
		return TRUE;
	};
};
...
{
	if((Kapitel >= 5) && (MadKillerCount >= 15) && (VatrasPissedOffForever == FALSE))
	{
		return TRUE;
	};
};
...
func void DIA_Addon_Vatras_Cavalorn_Bandit()
{
	...
	AI_Output(self,other,"DIA_Addon_Vatras_Cavalorn_Bandit_05_01");	//О Аданос! Это плохо. Это очень плохо.
...
func void DIA_Addon_Vatras_Cavalorn_Cavalorn()
{
	...
	AI_Output(self,other,"DIA_Addon_Vatras_Cavalorn_Cavalorn_05_01");	//Кавалорн? А где он?
...
func void DIA_Addon_Vatras_CavalornSentMe_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Vatras_Add_05_03");	//Ага... Значит, ты хочешь присоединиться к нам, сынок?
};
...
func void DIA_Vatras_INFLUENCE_FIRST_TRUTH()
{
	...
	{
		AI_Output(self,other,"DIA_Vatras_INFLUENCE_FIRST_TRUTH_05_02");	//Драконы? Ты говоришь о существах, которые до последнего времени упоминались только в легендах. Откуда ты знаешь это?
	};
	Info_ClearChoices(DIA_Addon_Vatras_WannaBeRanger);
...
func void DIA_Vatras_INFLUENCE_FIRST_LIE()
{
	...
	if(Vatras_First == 2)
	{
		AI_Output(self,other,"DIA_Vatras_INFLUENCE_FIRST_LIE_05_01");	//Ага. И кто сказал тебе это?
	}
	else
	{
...
func void DIA_Vatras_INFLUENCE_SECOND_TRUTH()
{
	...
	{
		AI_Output(self,other,"DIA_Vatras_INFLUENCE_SECOND_TRUTH_05_02");	//Некромант... Так он жив... И он послал тебя? А кто же ты тогда такой?
	};
	Info_ClearChoices(DIA_Addon_Vatras_WannaBeRanger);
...
func void DIA_Vatras_INFLUENCE_SECOND_LIE()
{
	...
	if(Vatras_Second == 2)
	{
		AI_Output(self,other,"DIA_Vatras_INFLUENCE_SECOND_LIE_05_01");	//Ты хотя бы помнишь, откуда ты пришел?
	}
	else
	{
...
func void DIA_Addon_Vatras_Free_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Vatras_Free_05_03");	//Аданос, благослови этого воина. Освети его путь и даруй ему силу противостоять любым опасностям.
	B_RaiseAttribute(other,ATR_MANA_MAX,ManaPoints(3));
...
func void DIA_Addon_Vatras_Waffen_Success()
{
	...
	if(Fernando_ImKnast == TRUE)
	{
		...
		{
			AI_Output(other,self,"DIA_Addon_Vatras_Waffen_Success_15_05");	//А теперь вы примете меня в Кольцо Воды?
			AI_Output(self,other,"DIA_Addon_Vatras_Waffen_Success_05_06");	//Мы не об ЭТОМ договаривались, и ты это знаешь.
		};
...
func void DIA_Addon_Vatras_SellStonplate_Info()
{
	...
	if(anzahl >= 10)
	{
		...
		B_RaiseAttribute(other,ATR_MANA_MAX,ManaPoints(anzahl * VatrasStoneplatesManaMultiplier));
		Npc_ChangeAttribute(other,ATR_MANA,ManaPoints(anzahl * VatrasStoneplatesManaMultiplier));
	}
	else if(anzahl >= 5)
	{
...
func void DIA_Vatras_Teach_Info()
{
	...
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Vatras_Teach_1);
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Vatras_Teach_5);
};

func void DIA_Vatras_Teach_BACK()
{
	if(other.attribute[ATR_MANA_MAX] >= T_HIGH * ManaTeachersLimitMultiplier)
	{
...
{
	B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(1),T_HIGH * ManaTeachersLimitMultiplier);
	Info_ClearChoices(DIA_Vatras_Teach);
	Info_AddChoice(DIA_Vatras_Teach,Dialog_Back,DIA_Vatras_Teach_BACK);
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Vatras_Teach_1);
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Vatras_Teach_5);
...
{
	B_TeachAttributePoints(self,other,ATR_MANA_MAX,ManaPoints(5),T_HIGH * ManaTeachersLimitMultiplier);
	Info_ClearChoices(DIA_Vatras_Teach);
	Info_AddChoice(DIA_Vatras_Teach,Dialog_Back,DIA_Vatras_Teach_BACK);
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(1),B_GetLearnCostAttribute(other,ATR_MANA_MAX)),DIA_Vatras_Teach_1);
	Info_AddChoice(DIA_Vatras_Teach,B_BuildLearnString(B_ManaPlusText(5),B_GetLearnCostAttribute(other,ATR_MANA_MAX) * 5),DIA_Vatras_Teach_5);
...
func void DIA_Vatras_GODS_INNOS()
{
	...
	AI_Output(self,other,"DIA_Vatras_GODS_INNOS_05_01");	//Хорошо. Иннос - первый и величайший из богов. Он создал солнце и мир.
