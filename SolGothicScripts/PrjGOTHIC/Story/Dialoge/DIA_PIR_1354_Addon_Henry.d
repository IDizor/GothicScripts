...
func void DIA_Addon_Henry_Malcom_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Henry_Malcom_04_01");	//Ну конечно... Я так и знал. Вечно он копается.
	B_MalcomExident();
};
...
func void DIA_Addon_Henry_Palisade_Bandits_Info()
{
	...
	if(self.aivar[AIV_PASSGATE] == FALSE)
	{
		AI_Output(self,other,"DIA_Addon_Henry_Palisade_Bandits_04_11");	//Жаль только, что тебе с ним поговорить не удастся. За вход-то ты не заплатил...
		Henry_Zoll_WhatFor = TRUE;
	};
};
...
func void DIA_Addon_Henry_Entercrew_Info()
{
	...
	if(self.aivar[AIV_PASSGATE] == FALSE)
	{
		AI_Output(self,other,"DIA_Addon_Henry_Entercrew_Add_04_00");	//Нет, так дело не пойдет, приятель!
...
func void DIA_Addon_Henry_WhatTeach_Info()
{
	...
	if(self.aivar[AIV_PASSGATE] == FALSE)
	{
		AI_Output(self,other,"DIA_Addon_Henry_WhatTeach_Add_04_01");	//Сначала заплати за проход в лагерь, а потом уже мы об этом поговорим.
