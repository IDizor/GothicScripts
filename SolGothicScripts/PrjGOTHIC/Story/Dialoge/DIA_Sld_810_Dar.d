...
func void DIA_Dar_Hallo_Ja()
{
	...
	AI_Output(self,other,"DIA_Dar_Hallo_Ja_03_03");	//У меня свои источники.
	Info_ClearChoices(DIA_Dar_Hallo);
};
...
func void DIA_Dar_DuDieb_Info()
{
	...
	AI_Output(self,other,"DIA_Dar_DuDieb_03_01");	//(смеется как идиот)
	AI_Output(other,self,"DIA_Dar_DuDieb_15_02");	//Ты ничего не знаешь об этом?
	AI_Output(self,other,"DIA_Dar_DuDieb_03_03");	//Нет.
	Dar_Verdacht = TRUE;
};
...
{
	AI_Output(other,self,"DIA_Dar_WoPaket_15_00");	//Где тюк?
...
func void DIA_Dar_ORCRING_necken()
{
	...
	AI_Output(self,other,"DIA_Dar_ORCRING_necken_03_01");	//Ох, ладно, заткнись. Ты-то вообще кто такой?
...
func void DIA_Dar_ORCRING_no()
{
	...
	AI_Output(self,other,"DIA_Dar_ORCRING_no_03_01");	//Конечно нет. Я бы очень удивился, если бы это было не так.
	Info_ClearChoices(DIA_Dar_ORCRING);
};
