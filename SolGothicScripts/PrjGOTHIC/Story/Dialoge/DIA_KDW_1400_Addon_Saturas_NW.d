...
func void DIA_Addon_Saturas_Hallo_weiЯtdu()
{
	...
	AI_Output(self,other,"DIA_Addon_Saturas_Hallo_weiЯtdu_14_04");	//Следите за языком, юноша!
	AI_Output(self,other,"DIA_Addon_Saturas_Hallo_weiЯtdu_14_05");	//Что, во имя всех стихий, тебе здесь нужно? 
...
func void DIA_Addon_Saturas_Hallo_Info()
{
	...
	Info_ClearChoices(DIA_Addon_Saturas_Hallo);
	Info_AddChoice(DIA_Addon_Saturas_Hallo,"Я...",DIA_Addon_Saturas_Hallo_Video);
};
...
func void DIA_Addon_Saturas_Ornament_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Saturas_Ornament_14_06");	//И к чему это нас привело, ответь? Хм-м, даже не думай, что я тебе доверяю...
...
func void DIA_Addon_Saturas_geheimbund_Info()
{
	...
	if(Lares_Angekommen == TRUE)
	{
		AI_Output(self,other,"DIA_Addon_Saturas_geheimbund_14_01");	//Что?! Как ты узнал об этом? Кто? Ларес? Я убью его!
		AI_Output(self,other,"DIA_Addon_Saturas_geheimbund_14_02");	//Он нарушил наше главное правило. О Кольце Воды говорить запрещено!
	};
	AI_Output(self,other,"DIA_Addon_Saturas_geheimbund_14_03");	//Не знаю, что мне следовало бы сказать. Просто я доверяю тебе, пока ты у меня на глазах.
	AI_Output(other,self,"DIA_Addon_Saturas_geheimbund_15_04");	//О Кольце мне сказал Ватрас.
	AI_Output(self,other,"DIA_Addon_Saturas_geheimbund_14_05");	//Что? Ватрас? Отлично! Из всех магов воды, которые есть на острове, ты встречаешься именно с тем, который тебя не знает.
	AI_Output(self,other,"DIA_Addon_Saturas_geheimbund_14_06");	//Предупреждаю, в этот раз не делай глупостей.
	AI_Output(other,self,"DIA_Addon_Saturas_geheimbund_15_07");	//Постараюсь.
...
func void DIA_Addon_Saturas_ScRanger_Info()
{
	...
	if(((SCIsWearingRangerRing == TRUE) && (RangerRingIsLaresRing == FALSE)) || (Hlp_IsItem(itm,ITAR_RANGER_Addon) == TRUE))
	{
		...
		AI_Output(self,other,"DIA_Addon_Saturas_ScRanger_14_04");	//Хорошо. Я приму тебя в наши ряды.
	...
	if(SaturasKnows_SC_IsRanger == TRUE)
	{
		...
		if((RangerMeetingRunning == LOG_SUCCESS) && (Npc_HasItems(other,ItWr_Vatras2Saturas_FindRaven) || Npc_HasItems(other,ItWr_Vatras2Saturas_FindRaven_opened)))
		{
			...
			if(Vatras2Saturas_FindRaven_Open == TRUE)
			{
				...
				AI_Output(self,other,"DIA_Addon_Saturas_OpenPortal_14_06");	//Надеюсь, мое и так хрупкое доверие к тебе не пошатнется окончательно. Ради твоего же блага.
			}
			else
			{
				AI_Output(self,other,"DIA_Addon_Saturas_OpenPortal_14_07");	//Интересно. Хорошо.
