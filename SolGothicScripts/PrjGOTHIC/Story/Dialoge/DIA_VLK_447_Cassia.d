...
func void DIA_Cassia_Vorteil_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Vorteil_16_03");	//Нет. Тебе только нужно придерживаться наших правил. Вот и все.
};
...
func void DIA_Cassia_Ablehnen_Info()
{
	...
	if(MIS_ThiefGuild_sucked == FALSE)
	{
		...
		AI_Output(self,other,"DIA_Cassia_Ablehnen_16_02");	//И даже не думай, чтобы предать нас. Ты горько пожалеешь об этом.
...
func void DIA_Cassia_Ablehnen_Okay()
{
	...
	AI_Output(self,other,"DIA_Cassia_Ablehnen_Okay_16_01");	//Ты принял мудрое решение. Если ты сможешь доказать серьезность своих намерений, то сможешь влиться в наши ряды.
...
func void DIA_Cassia_Aufnahme_Info()
{
	...
	AI_Output(self,other,"DIA_Cassia_Aufnahme_16_02");	//Возьми этот ключ. Он открывает дверь в отель. Теперь тебе не придется каждый раз плыть сюда.
...
func void DIA_Cassia_abgeben_Info()
{
	...
	if(B_GiveInvItems(other,self,ItMi_BloodCup_MIS,6))
	{
		...
		AI_Output(self,other,"DIA_Cassia_abgeben_16_04");	//Мне больше нечего тебе предложить, но ты можешь учиться у меня, когда захочешь. Ну и всегда помни - на этом острове много что плохо лежит...
