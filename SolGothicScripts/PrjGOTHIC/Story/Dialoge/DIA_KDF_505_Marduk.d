...
func void DIA_Marduk_Kap3_Hello_NotYourConcern()
{
	...
	if(hero.guild == GIL_PAL)
	{
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_01");	//Паладин всегда должен быть вежливым и скромным. Ты должен защищать тех, кто не может защитить себя сам.
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_02");	//Это привилегия, и ты должен быть благодарен, что Иннос дает тебе такую возможность. Подумай об этом!
	};
	if(hero.guild == GIL_DJG)
	{
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_03");	//Эх, раньше всякий сброд не пускали в монастырь. Твое поведение доказывает, что это были хорошие времена.
		AI_Output(self,other,"DIA_Marduk_Kap3_Hello_NotYourConcern_05_04");	//Я предупреждаю тебя, не стоит сеять тут смуту - ты будешь немедленно наказан. Мы не будем проявлять фальшивое великодушие.
	};
	Info_ClearChoices(DIA_Marduk_Kap3_Hello);
};
...
func void DIA_Marduk_Kap3_PERM_Info()
{
	...
	AI_Output(self,other,"DIA_Marduk_Kap3_PERM_05_04");	//Враг, по-видимому, уже вошел в город.
	...
	AI_Output(self,other,"DIA_Marduk_Kap3_PERM_05_07");	//Прямо средь бела дня! Это зашло слишком далеко, но я боюсь, что это только начало.
