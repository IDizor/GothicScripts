...
func void DIA_Orlan_Wein_Nie()
{
	...
	AI_Output(self,other,"DIA_Orlan_Wein_Nie_05_01");	//Ты не хочешь вести со мной бизнес, да? Ну хорошо, вот твое золото.
...
{
	if(Npc_KnowsInfo(other,DIA_Orlan_WERBISTDU) && (DIA_Orlan_RUESTUNG_noPerm == FALSE) && (other.guild == GIL_NONE))
	{
		return TRUE;
	};
};
...
func void DIA_Orlan_RUESTUNG_Info()
{
	...
	Info_AddChoice(DIA_Orlan_RUESTUNG,"Купить кожаные доспехи: 25/20/5/0. Цена: 250 золота.",DIA_Orlan_RUESTUNG_Buy);
};
...
func void DIA_Orlan_MieteFaellig_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Orlan_MieteFaellig_05_00");	//Я очень рад визиту такого гостя. Оставайся здесь, сколько пожелаешь. Это честь для меня.
