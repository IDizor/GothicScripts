...
func void DIA_Addon_Emilio_Attentat_Info()
{
	...
	if(Emilio_Switch == 0)
	{
		AI_Output(self,other,"DIA_Addon_Emilio_Attentat_10_01");	//Эй, приятель, я не хочу ничего об этом знать!
		Emilio_Switch = 1;
	}
	else
	{
		AI_Output(self,other,"DIA_Addon_Emilio_Attentat_10_02");	//ВООБЩЕ НИЧЕГО!
		Emilio_Switch = 0;
	};
};
...
func void DIA_Addon_Emilio_Senyan_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Addon_BDT_10015_Emilio_Senyan_10_00");	//Скажи мне, ПОЧЕМУ Сеньян закричал: 'Посмотрите, кто пришел'?
		AI_Output(other,self,"DIA_Addon_BDT_10015_Emilio_Senyan_15_01");	//Невыплаченные долги.
	}
	else
	{
	...
	AI_Output(self,other,"DIA_Addon_BDT_10015_Emilio_Senyan_10_04");	//Нет, приятель, у меня к тебе по этому поводу никаких претензий.
	AI_Output(self,other,"DIA_Addon_BDT_10015_Emilio_Senyan_10_05");	//Даже наоборот. Этот ублюдок работал на Эстебана.
...
func void DIA_Addon_Emilio_Jetzt_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Emilio_Jetzt_10_01");	//Я был там достаточно долго и вкалывал, пока не стал валиться с ног от усталости. Теперь мне надо отдохнуть несколько дней.
	AI_Output(self,other,"DIA_Addon_Emilio_Jetzt_10_02");	//Прежде, чем я получу следующий красный камень.
};
...
func void DIA_Addon_Emilio_VonEmilio_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Emilio_VonEmilio_10_03");	//Я... ничего не знаю!
	if(!Npc_IsDead(Senyan))
	{
...
func void DIA_Addon_Emilio_HilfMir_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Emilio_HilfMir_10_03");	//Я... черт! Я скажу тебе одно имя. И больше ничего.
