...
func void DIA_Raoul_NoSentenza_Info()
{
	...
	if(Hlp_IsValidNpc(Sentenza) && !C_NpcIsDown(Sentenza))
	{
		...
		AI_Output(self,other,"DIA_Raoul_NoSentenza_01_03");	//Подожди секундочку, сейчас он подойдет!
...
func void DIA_Raoul_Hello_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Raoul_Hello_01_00");	//Чего тебе нужно?
	}
	else
	{
		AI_Output(self,other,"DIA_Raoul_Hello_01_01");	//Что тебе нужно на этот раз?
	};
};
...
{
	AI_Output(self,other,"DIA_Raoul_TROLL_01_00");	//Только посмотрите на это!
...
func void DIA_Raoul_TROLL_rechnung()
{
	...
	AI_Output(self,other,"DIA_Raoul_TROLL_rechnung_01_05");	//Что ты хочешь сказать этим 'и что'?
...
func void DIA_Raoul_Ship_Info()
{
	...
	AI_Output(self,other,"DIA_Raoul_Ship_01_01");	//Что ты замышляешь? Ты хочешь захватить корабль паладинов?
	AI_Output(other,self,"DIA_Raoul_Ship_15_02");	//А что если и так?
	AI_Output(self,other,"DIA_Raoul_Ship_01_03");	//У тебя совсем крыша поехала. Нет, спасибо. Это не для меня.
