...
func int DIA_Addon_Morgan_EXIT_Info()
{
	if(GregIsBack == FALSE)
	{
		...
		{
			AI_Output(self,other,"DIA_Addon_Morgan_Perm_07_01");	//Отличная мысль.
			Morgan_Perm_Counter = 1;
		}
		else if(Morgan_Perm_Counter == 1)
		{
			AI_Output(self,other,"DIA_Addon_Morgan_Perm_07_02");	//Что ж, спокойной ночи.
			Morgan_Perm_Counter = 2;
		}
		else if(Morgan_Perm_Counter == 2)
		{
			AI_Output(self,other,"DIA_Addon_Morgan_Perm_07_03");	//С огромным удовольствием.
...
func int DIA_Addon_Morgan_Anheuern_Info()
{
	...
	{
		AI_Output(self,other,"DIA_Addon_Morgan_Anheuern_07_01");	//Удачи в потрошении монстров!
	};
	return TRUE;
};
...
func void DIA_Addon_Morgan_Hello_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Morgan_Hello_07_01");	//А? Что тебе нужно?
	AI_Output(self,other,"DIA_Addon_Morgan_Hello_07_02");	//Аллигатор Джек уже вернулся?
	AI_Output(self,other,"DIA_Addon_Morgan_Hello_07_03");	//Нет? Ну, значит, скоро вернется. Приятных снов.
	AI_StopProcessInfos(self);
};
...
func void DIA_Addon_Morgan_Meat_Info()
{
	...
	if(GregIsBack == FALSE)
	{
		AI_Output(self,other,"DIA_Addon_Morgan_Meat_07_01");	//А-а-а. Мне нужно глотнуть рома, чтобы проснуться.
...
func void DIA_Addon_Morgan_Job_Info()
{
	...
	if(GregIsBack == FALSE)
	{
		AI_Output(self,other,"DIA_Addon_Morgan_Job_07_05");	//Я сказал им, что к возвращению Грега здесь не должно быть ни одного зверя.
	};
};
...
func void DIA_Addon_Morgan_JoinMorgan_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Morgan_JoinMorgan_07_01");	//Мой отряд? Мой отряд сейчас валяется на пляже.
	...
	AI_Output(self,other,"DIA_Addon_Morgan_JoinMorgan_07_06");	//Разберись с ними, и ты заработаешь уважение людей...
...
func void DIA_Addon_Morgan_FOUNDTHEM_NoRing()
{
	...
	AI_Output(self,other,"DIA_Addon_Morgan_FOUNDTHEM_NoRing_07_01");	//Что? Возвращайся и поищи получше. Оно должно было быть у Ангуса.
...
func void DIA_Addon_Morgan_FOUNDTHEM_GiveRing()
{
	...
	AI_Output(self,other,"DIA_Addon_Morgan_FOUNDTHEM_GiveRing_07_01");	//Да, это оно! Спасибо тебе!
...
func void DIA_Addon_Morgan_Francis_Info()
{
	...
	AI_Output(self,other,"DIA_Addon_Morgan_Francis_07_01");	//Я ничего не имею против него, пока он ко мне не лезет!
	AI_Output(other,self,"DIA_Addon_Morgan_Francis_15_02");	//Он здесь начальник?
	AI_Output(self,other,"DIA_Addon_Morgan_Francis_07_03");	//Он ДУМАЕТ, что он главный.
