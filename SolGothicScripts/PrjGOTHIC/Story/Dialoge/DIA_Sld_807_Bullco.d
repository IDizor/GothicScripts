...
func void DIA_Bullco_WontLeave_Info()
{
	...
	if(self.aivar[AIV_DefeatedByPlayer] == FALSE)
	{
		AI_Output(self,other,"DIA_Bullco_WontLeave_06_01");	//Похоже, нам придется обсудить этот вопрос опять.
		AI_StopProcessInfos(self);
		B_Attack(self,other,AR_NONE,1);
	}
	else
	{
		AI_Output(self,other,"DIA_Bullco_WontLeave_06_02");	//Да, да, иди, расскажи это кому-нибудь, кому это интересно.
		Bullco_scharf = FALSE;
		AI_StopProcessInfos(self);
	};
};
...
func void DIA_Bullco_PepesSchafe_Info()
{
	...
	AI_Output(self,other,"DIA_Bullco_PepesSchafe_06_08");	//Ты, ты...
	AI_Output(other,self,"DIA_Bullco_PepesSchafe_15_09");	//Да?
	AI_Output(self,other,"DIA_Bullco_PepesSchafe_06_10");	//Мерзкий коварный ублюдок...
	Bullco_scharf = FALSE;
