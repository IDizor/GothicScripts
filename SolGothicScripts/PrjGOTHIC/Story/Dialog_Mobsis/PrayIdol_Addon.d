...
func void PC_PrayIdol_PrayIdol_Info()
{
	...
	if(GivenMana <= 10)
	{
		if(hero.attribute[ATR_MANA_MAX] > 5)
		{
...
func void PC_PrayIdol_PrayIdol_BigPayManaPay()
{
	...
	{
		B_GetBeliarsGold(100);
	}
	else
	{
		B_GetBeliarsGold(200);
	};
	PrayIdolDay = Wld_GetDay();
	Info_ClearChoices(PC_PrayIdol_PrayIdol);
};
...
{
	if((Npc_KnowsInfo(other,DIA_Addon_Xardas_AddonSuccess)) && (PLAYER_MOBSI_PRODUCTION == MOBSI_PRAYIDOL) && (C_ScCanUpgrateBeliarsWeapon() == TRUE) && (C_ScHasBeliarsWeapon() == TRUE))
	{
		return TRUE;
	};
};
